/* ===========================================================
 * trumbowyg.upload.js (Modified for CB Platform)
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (data) {
                                            try{ // Попытка обработки ответа
                                                var _resp = JSON.parse(data); // Попытка парсинга JSON
                                                if(!_resp.complete){ // Сервер выдал ошибку
                                                    ...
                                                    trumbowyg.execCmd('insertImage', _resp.url);
                                                    $('img[src="' + _resp.url + '"]:not([alt])', trumbowyg.$box).attr('alt', values.alt);
                                                    setTimeout(function () {
                                                        trumbowyg.closeModal();
                                                    }, 250);
                                                    //trumbowyg.$c.trigger('tbwuploadsuccess', [trumbowyg, data, _resp.url]);
                                                }
                                            }catch(e){ // Ошибка
                                                console.log("CB API Upload Error: Failed to convert server response"); // Вывод ошибки в консоль
                                                console.log(data); // Вывод контента
                                                trumbowyg.addErrorOnModalField(
                                                    $('input[type=file]', $modal),
